...
export const automateCommand = new Command("automate")
...
automateCommand
    ...
    .action(async (options) => {
        ...
        try {
            ...
            const res = await getWithAuth(`/api/automation-rules?repositoryId=${repoRes.repository.id}`);
            ...
            for (const action of answers.actions) {
                ...
                if (action === "add_label") {
                    ...
                    const { reviewer } = await inquirer.prompt([
                        { type: "input", name: "reviewer", message: "Reviewer username (@codeowners for auto):" },
                    ]);
...
async function toggleRule(name: string, enabled: boolean) {
    ...
    try {
        ...
        await patchWithAuth(`/api/automation-rules?id=${rule.id}`, { isEnabled: enabled });

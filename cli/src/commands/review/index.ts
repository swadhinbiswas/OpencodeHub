...
export const reviewCommands = new Command("review")
    ...
    .action(async (prNumber: string, options) => {
        ...
        try {
            ...
            if (options.wait) {
                ...
                console.log(chalk.dim("  Run 'och review status " + prNumber + "' to check progress"));
            }
        } catch (error) {
...
reviewCommands
    ...
    .action(async (prNumber: string) => {
        ...
        const spinner = ora(`Fetching review status for PR #${prNumber}...`).start();
...
function displayReview(review: AIReview) {
    ...
    if (review.overallSeverity) {
        console.log(`Severity: ${getSeverityIcon(review.overallSeverity)} ${review.overallSeverity}`);
    }
    ...
    if (review.suggestions && review.suggestions.length > 0) {
        ...
        for (const suggestion of review.suggestions) {
            ...
            console.log(chalk.dim(`  ${suggestion.path}${suggestion.line ? `:${suggestion.line}` : ""}`));

...
export const ciCommands = new Command("ci")
    ...
    .action(async (options) => {
        ...
        try {
            ...
            for (const run of result.data) {
                ...
                console.log(`${icon} #${run.number} ${run.event} on ${chalk.cyan(run.branch)}`);
                console.log(chalk.dim(`  ${run.commit.slice(0, 7)} • ${time} • ${duration}`));
            }
...
ciCommands
    ...
    .action(async () => {
        ...
        try {
            ...
            console.log(chalk.dim(`  ${run.commit.slice(0, 7)} • ${formatDuration(run.duration)}`));
            console.log("");
        } catch (error) {

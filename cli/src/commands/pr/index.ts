...
import { getWithAuth, postWithAuth, patchWithAuth, deleteWithAuth } from "../../lib/api.js";
...
export const prCommands = new Command("pr")
...
prCommands
    ...
    .option("-l, --label <label>", "Add label (can be used multiple times)", (val, acc: string[]) => [...acc, val], [])
    .action(async (options) => {
        ...
        try {
            ...
            if (currentBranch === options.base) {
                spinner.fail(`Cannot create PR from ${options.base} to ${options.base}`);
                process.exit(1);
            }
            ...
            if (!title) {
                ...
                const defaultTitle = lastCommit.latest?.message?.split("\n")[0] || currentBranch;
            ...
            spinner.succeed(`Created PR #${result.data.number}: ${result.data.title}`);
            console.log(chalk.dim(`  ${config.serverUrl}/${repoInfo.owner}/${repoInfo.repo}/pull/${result.data.number}`));
        } catch (error) {
    ...
    .action(async (options) => {
        ...
        try {
            ...
            for (const pr of result.data) {
                const stateIcon = pr.state === "open" ? chalk.green("●") :
...
prCommands
    ...
    .action(async (number: string, options) => {
        ...
        try {
            ...
            const result = await getWithAuth<{ data: PullRequest & { body?: string } }>(
            ...
            console.log(chalk.bold(`\n#${pr.number} ${pr.title}\n`));
            console.log(`State: ${stateColor(pr.state.toUpperCase())}${pr.isDraft ? chalk.dim(" (draft)") : ""}`);
            ...
            if (pr.additions !== undefined) {
                console.log(`Changes: ${chalk.green(`+${pr.additions}`)} ${chalk.red(`-${pr.deletions}`)} (${pr.changedFiles} files)`);
            }
            ...
            console.log(chalk.dim("\n─".repeat(40)));
            console.log(chalk.dim(`${config.serverUrl}/${repoInfo.owner}/${repoInfo.repo}/pull/${number}`));
            console.log("");
        } catch (error) {

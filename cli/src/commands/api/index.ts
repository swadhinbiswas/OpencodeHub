...
export const apiCommands = new Command("api")
    ...
    .option("-F, --field <field>", "Add field to request body (key=value)", (val, acc: string[]) => [...acc, val], [])
    .option("-f, --raw-field <field>", "Add raw field (no JSON parsing)", (val, acc: string[]) => [...acc, val], [])
    .option("-H, --header <header>", "Add header", (val, acc: string[]) => [...acc, val], [])
    ...
    .action(async (endpoint: string, options) => {
        ...
        try {
            ...
            if (options.include) {
                console.log(chalk.cyan(`HTTP/1.1 ${response.status} ${response.statusText}`));

...
export const releaseCommands = new Command("release")
...
releaseCommands
    ...
    .action(async (tag: string, options) => {
        ...
        try {
            ...
            const status = result.data.isDraft ? " (draft)" : result.data.isPrerelease ? " (pre-release)" : "";
            spinner.succeed(`Created release ${chalk.green(tag)}${status}`);
            console.log(chalk.dim(`  ${config.serverUrl}/${repoInfo.owner}/${repoInfo.repo}/releases/tag/${tag}`));
        } catch (error) {
    ...
    .action(async (options) => {
        ...
        try {
            ...
            for (const release of result.data) {
                const icon = release.isDraft ? chalk.yellow("◌") :
                    release.isPrerelease ? chalk.blue("●") :
                        chalk.green("●");
                const labels = [
                ...
                console.log(`${icon} ${chalk.bold(release.tagName)} ${release.name !== release.tagName ? `- ${release.name}` : ""} ${labels}`);
                console.log(chalk.dim(`  ${new Date(release.publishedAt || release.createdAt).toLocaleDateString()} • @${release.author.username}`));
            }
...
releaseCommands
    ...
    .action(async (tag: string) => {
        ...
        try {
            ...
            console.log(`Date: ${new Date(release.publishedAt || release.createdAt).toLocaleString()}`);

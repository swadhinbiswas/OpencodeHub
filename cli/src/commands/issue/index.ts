...
export const issueCommands = new Command("issue")
...
issueCommands
    ...
    .option("-l, --label <label>", "Add label", (val, acc: string[]) => [...acc, val], [])
    ...
    .action(async (options) => {
        ...
        try {
            ...
            spinner.succeed(`Created issue #${result.data.number}: ${result.data.title}`);
            console.log(chalk.dim(`  ${config.serverUrl}/${repoInfo.owner}/${repoInfo.repo}/issues/${result.data.number}`));
        } catch (error) {
    ...
    .action(async (options) => {
        ...
        try {
            ...
            for (const issue of result.data) {
                ...
                const labels = issue.labels?.map(l => chalk.hex(l.color || "#888")(l.name)).join(" ") || "";
...
issueCommands
    ...
    .action(async (number: string, options) => {
        ...
        try {
            ...
            console.log(chalk.dim("\n─".repeat(40)));
            console.log(chalk.dim(`${config.serverUrl}/${repoInfo.owner}/${repoInfo.repo}/issues/${number}`));
            console.log("");
        } catch (error) {

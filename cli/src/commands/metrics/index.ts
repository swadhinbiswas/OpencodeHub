...
export const metricsCommands = new Command("metrics")
    .description("View developer metrics");
...
function displayUserMetrics(metrics: UserMetrics, username: string) {
    ...
    console.log(`  Avg Merge: ${formatDuration(metrics.authored.avgTimeToMerge)}`);
    ...
    if (metrics.trends && metrics.trends.length > 0) {
        ...
        for (const week of metrics.trends) {
            ...
            console.log(`    Authored: ${progressBar(week.authored, maxAuthored)} ${week.authored}`);
            console.log(`    Reviewed: ${progressBar(week.reviewed, maxReviewed)} ${week.reviewed}`);
        }
    }
...
function displayRepoMetrics(metrics: RepoMetrics[], repoName: string) {
    ...
    console.log(`  Avg First Review: ${formatDuration(latest.avgTimeToFirstReview)}`);
    ...
    if (latest.topAuthors && latest.topAuthors.length > 0) {
        ...
        for (const author of latest.topAuthors.slice(0, 5)) {
            console.log(`  @${author.username.padEnd(15)} ${progressBar(author.prCount, maxPRs, 15)} ${author.prCount}`);
        }
    }
    ...
    if (latest.topReviewers && latest.topReviewers.length > 0) {
        console.log(chalk.bold("\nTop Reviewers"));
        const maxReviews = Math.max(...latest.topReviewers.map(r => r.reviewCount), 1);
        for (const reviewer of latest.topReviewers.slice(0, 5)) {
            console.log(`  @${reviewer.username.padEnd(15)} ${progressBar(reviewer.reviewCount, maxReviews, 15)} ${reviewer.reviewCount}`);
        }
    }
    ...
    if (metrics.length > 1) {
        ...
        for (const week of metrics.slice(0, 4)) {
            console.log(`  ${week.period}: ${progressBar(week.prsMerged, maxMerged)} ${week.prsMerged} merged`);
        }
    }

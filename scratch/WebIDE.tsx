...
import { Folder, File, ChevronRight, ChevronDown, GitCommit, X, GitBranch, RefreshCw, XCircle, AlertTriangle, Layout, Search, Settings, Home, Activity } from "lucide-react";
...
export function WebIDE({ owner, repo, defaultBranch, currentUser }: WebIDEProps) {
    ...
    useEffect(() => {
        const fetchTree = async () => {
            try {
                const res = await fetch(`/api/repos/${owner}/${repo}/ide/fs?action=tree&branch=${defaultBranch}`);
    ...
    const handleOpenFile = async (path: string) => {
        if (!openFiles.find(f => f.path === path)) {
            // fetch content
            try {
                const res = await fetch(`/api/repos/${owner}/${repo}/ide/fs?action=file&branch=${defaultBranch}&path=${encodeURIComponent(path)}`);
                const data = await res.json();

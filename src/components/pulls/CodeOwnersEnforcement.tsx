...
export function CodeOwnersEnforcement({ repoOwner, repoName, prNumber }: CodeOwnersEnforcementProps) {
  ...
  if (isLoading) {
    return (
      <div className="mt-4 border rounded-xl overflow-hidden bg-card/80 backdrop-blur-md shadow-sm border-border animate-pulse">
        <div className="bg-white/[0.02] p-3 border-b border-border flex items-center">
          <div className="h-4 w-32 bg-secondary/80 rounded"></div>
        </div>
  ...
  if (error || !data) {
    return (
      <div className="mt-4 border rounded-xl overflow-hidden bg-card/80 backdrop-blur-md shadow-sm border-red-500/20">
  ...
  return (
    <div className={`mt-4 border rounded-xl overflow-hidden backdrop-blur-md shadow-sm ${
      ...
      <div className="bg-white/[0.02] p-3 border-b border-border flex items-center justify-between">
        ...
        {enforced ? (
          ...
          <span className="text-xs px-2 py-0.5 rounded-full border bg-secondary border-border text-muted-foreground font-medium">
            Optional
          </span>
        )}
      </div>
        ...
        {rules && rules.length > 0 ? (
          <div className="space-y-3">
            {rules.map((rule: any, idx: number) => {
              ...
              return (
                <div key={idx} className="bg-secondary rounded-lg border border-white/5 p-3 flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <div className="font-mono text-xs text-muted-foreground break-all bg-black/30 px-2 py-1 rounded">
                      {rule.pattern}
                    </div>
                    {ruleSatisfied ? (
                  ...
                  <div className="text-xs text-muted-foreground flex items-center gap-1.5 flex-wrap">
                    ...
                    {rule.owners.map((owner: string) => (
                      <span key={owner} className="flex items-center gap-1 text-muted-foreground font-medium">

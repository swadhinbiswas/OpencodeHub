...
export const POST: APIRoute = withErrorHandler(async ({ request, params }) => {
    ...
    try {
            ...
            try {
                ...
                if (force) {
                    try {
                        ...
                        logger.warn({ ref, err: fetchError }, "Failed to force fetch ref");
                    }
                }
            }
        }
        ...
        logger.info({ repoId: repository.id, userId: tokenPayload.userId, refs }, "Git push received and processed");

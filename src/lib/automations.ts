...
const RETRYABLE_ACTIONS = new Set<AutomationAction["type"]>([
  "notify_slack",
  "notify_discord",
  "notify_teams",
  "trigger_ai_review",
...
const DEFAULT_RETRYABLE_MAX_ATTEMPTS = 3;

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function getMaxAttemptsForAction(action: AutomationAction): number {
  const requested = Number(action.params.maxAttempts);
  if (Number.isInteger(requested) && requested >= 1 && requested <= 5) {
    return requested;
  }
  return RETRYABLE_ACTIONS.has(action.type)
    ? DEFAULT_RETRYABLE_MAX_ATTEMPTS
    : DEFAULT_MAX_ATTEMPTS;
}

async function recordDeadLetterExecution(options: {
  ruleId: string;
  executionId: string;
  ...
  try {
    ...
    await db.insert(schema.auditLogs).values({
      ...
      targetId: options.executionId,
      targetName: options.ruleId,
      data: JSON.stringify({
    ...
    logger.error(
      ...
      "Failed to record automation dead-letter audit log",
    );
  }
}
    ...
    for (const rule of rules) {
      ...
      try {
        ...
        await db.insert(schema.automationExecutions).values({
          ...
          triggeredByType: context.pullRequestId
            ? "pull_request"
            : context.issueId
              ? "issue"
              : "other",
        ...
        for (const action of actions) {
            ...
            try {
              ...
              lastError =
                error instanceof Error
                  ? error
                  : new Error("Unknown action execution error");
              const isLastAttempt = attempt === maxAttempts;
              if (!isLastAttempt) {
                // Small bounded backoff to improve external API reliability without stalling workers.
          ...
          if (lastError) {
            ...
            await recordDeadLetterExecution({
              ruleId: rule.id,
              executionId,
...
function evaluateCondition(
  ...
  switch (operator) {
    ...
    case "matches":
      if (typeof actual === "string" && typeof expected === "string") {
        try {
          // Safety: limit pattern length and apply timeout to prevent ReDoS
          if (expected.length > 200) return false;
          const regex = new RegExp(expected);
          return regex.test(actual.slice(0, 10_000));
        } catch {
  ...
  switch (action.type) {
    case "add_label": {
      if (context.pullRequestId && action.params.label) {
        // Add label to PR
        await db
          .insert(schema.pullRequestLabels)
          .values({
            id: generateId(),
        ...
        await db
          ...
          .where(
            and(
              ...
              eq(
                schema.pullRequestLabels.labelId,
        ...
        if (assignee === "@codeowners") {
          try {
            ...
            if (pr && pr.repository) {
              ...
              const { CODEOWNERS_PATHS, getSuggestedReviewers } =
                await import("./codeowners");
              ...
              if (codeOwnersContent) {
                ...
                for (const owner of owners) {
                  ...
                  if (user) {
                    await db
                      .insert(schema.pullRequestReviewers)
                      .values({
                        id: generateId(),
                        pullRequestId: context.pullRequestId,
                        userId: user.id,
      ...
      if (context.pullRequestId) {
        ...
        if (pr) {
          await addToMergeQueue({
            ...
            mergeMethod:
              (action.params.method as "merge" | "squash" | "rebase") ||
              "squash",
          });
        }
      }
    ...
    case "trigger_iac_hook": {
      if (context.pullRequestId) {
        ...
        if (pr) {
          const actionType =
            (action.params.action as "plan" | "apply") || "plan";
          const { triggerIaCHooks } = await import("./iac-hooks");
          await triggerIaCHooks({
            repositoryId: pr.repositoryId,
            action: actionType,
            runId:
              typeof action.params.runId === "string"
                ? action.params.runId
                : undefined,
            message:
              typeof action.params.message === "string"
                ...
                : undefined,
            configId:
              typeof action.params.configId === "string"
                ? action.params.configId
                : undefined,
          });
        }
      }
    ...
    case "trigger_cloud_deploy": {
      if (context.pullRequestId) {
        const pr = await db.query.pullRequests.findFirst({
          where: eq(schema.pullRequests.id, context.pullRequestId),
        });
        if (pr) {
          const { triggerCloudDeploy } = await import("./cloud-hooks");
          await triggerCloudDeploy({
            repositoryId: pr.repositoryId,
            configId:
              typeof action.params.configId === "string"
                ? action.params.configId
                : undefined,
            imageTag:
              typeof action.params.imageTag === "string"
                ? action.params.imageTag
                : undefined,
            namespace:
              typeof action.params.namespace === "string"
                ? action.params.namespace
                : undefined,
            deploymentName:
              typeof action.params.deploymentName === "string"
                ? action.params.deploymentName
                : undefined,
            serviceName:
              typeof action.params.serviceName === "string"
                ? action.params.serviceName
                : undefined,
            appName:
              typeof action.params.appName === "string"
                ? action.params.appName
                : undefined,
            taskDefinition:
              typeof action.params.taskDefinition === "string"
                ? action.params.taskDefinition
                : undefined,
            clusterName:
              typeof action.params.clusterName === "string"
...
export async function getAutomationRules(repositoryId: string) {
  ...
  return db.query.automationRules.findMany({
    ...
    orderBy: [
      desc(schema.automationRules.priority),
      desc(schema.automationRules.createdAt),
    ],
  });
}

...
export async function createSubTask(options: {
  ...
  logger.info(
    { taskId: task.id, parentId: options.parentId },
    "Sub-task created",
  );
...
export async function getEpicInfo(epicId: string): Promise<EpicInfo | null> {
  ...
  const epic = await db.query.issues.findFirst({
    where: and(eq(schema.issues.id, epicId), eq(schema.issues.type, "epic")),
  });
...
export async function getRepositoryEpics(
  ...
  // Batch fetch all children for all epics in one query
  const epicIds = epics.map((e) => e.id);
  const allChildren = await db.query.issues.findMany({
    where: inArray(schema.issues.parentId, epicIds),
  });

  // Group children by parent in memory
  const childrenByEpic = new Map<string, typeof allChildren>();
  for (const child of allChildren) {
    if (!child.parentId) continue;
    const list = childrenByEpic.get(child.parentId) || [];
    list.push(child);
    childrenByEpic.set(child.parentId, list);
  }
 ...
 */
export async function addToEpic(
  issueId: string,
  epicId: string,
): Promise<boolean> {
  const db = getDatabase();

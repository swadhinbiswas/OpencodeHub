 ...
 */
export async function getAdvertisedRefs(
...
/**
 * Handles git-upload-pack (Clone/Fetch) — with process timeout
 */
export async function handleUploadPack(
  repoPath: string,
  reqBody: Readable | ReadableStream,
): Promise<Readable> {
  const nodeStream = isReadable(reqBody)
    ? reqBody
    : Readable.fromWeb(reqBody as any);
  ...
  const timer = setTimeout(() => {
    logger.warn({ repoPath }, "git upload-pack timed out, killing process");
    child.kill("SIGKILL");
...
class PackStreamProcessor extends Writable {
  ...
  private async init(firstChunk: any) {
    ...
    this.uploadPromise = this.storage.writeStream(
      this.storageTempKey,
      this.storagePass,
    );
    ...
    this.indexPackPromise = new Promise<void>((resolve, reject) => {
      ...
      this.indexPack!.stderr!.on("data", (d: any) =>
        logger.debug(`[index-pack] ${d}`),
      );
...
export async function handleReceivePack(
  ...
  const inputStream = isReadable(reqBody)
    ? reqBody
    : Readable.fromWeb(reqBody as any);

  // Enforce pack size limit via a metering transform
  let receivedBytes = 0;
  const sizeLimiter = new Transform({
    transform(chunk, _encoding, callback) {
      receivedBytes += chunk.length;
      if (receivedBytes > MAX_PACK_SIZE_BYTES) {
        callback(
          new Error(
            `Pack exceeds maximum allowed size of ${MAX_PACK_SIZE_BYTES / (1024 * 1024)} MB`,
          ),
        );
        return;
      }
  ...
  const limitedStream = inputStream.pipe(sizeLimiter);

  const { commands, packs } = splitReceivePackStream(limitedStream);
  ...
  if (packProcessor.packHash) {
    ...
    const { tempKey, packKey, idxKey, idxPath } =
      packProcessor.getFinalPaths(hash);
    ...
    try {
      ...
      try {
        await fs.copyFile(sourcePack, targetPack);
      } catch {}
    }

    // Index file is definitely in .tmp
    try {
      await fs.copyFile(idxPath, targetIdx);
    } catch {}
...
class PktLineDemuxer extends Transform {
  ...
  constructor(
    private options: {
      onCommand: (data: Buffer) => void;
      onPack: (data: Buffer) => void;
    },
  ) {
    super();
  }
...
function splitReceivePackStream(stream: Readable): {
  commands: Readable;
  packs: Readable;
} {
...
function reportStatus(commandBuffs: string[], error?: string): Readable {
  ...
  for (const line of commandBuffs) {
    ...
    if (parts.length >= 3) {
      ...
      if (!seen.has(refName)) {
        s.push(
          sidebandMsg(error ? `ng ${refName} ${error}\n` : `ok ${refName}\n`),
        );
        seen.add(refName);
      }
    }
...
function isReadable(stream: any): stream is Readable {
  return (
    stream instanceof Readable || (stream && typeof stream.on === "function")
  );
}

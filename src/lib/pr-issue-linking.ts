...
export function parseIssueReferences(text: string): {
  ...
  const referenceTypeByNumber = new Map<
    number,
    "closes" | "fixes" | "relates"
  >();
  const setReference = (
    number: number,
    type: "closes" | "fixes" | "relates",
...
> {
  ...
  try {
    const links =
      (await db.query.prIssueLinks?.findMany({
        where: eq(schema.prIssueLinks.pullRequestId, pullRequestId),
      })) || [];

    if (links.length === 0) return [];

    // Batch fetch all linked issues in one query
    ...
    return links
      .filter((link) => issueMap.has(link.issueId))
      .map((link) => ({ link, issue: issueMap.get(link.issueId)! }));
  } catch {
    return [];
  }
}
...
> {
  ...
  try {
    ...
    // Batch fetch all linked PRs in one query
    ...
    return links
      .filter((link) => prMap.has(link.pullRequestId))
      .map((link) => ({ link, pr: prMap.get(link.pullRequestId)! }));
  } catch {
    return [];
  }
}
...
export async function closeLinkedIssuesOnMerge(
  ...
  try {
    ...
    if (closedIssueIds.length > 0) {
      logger.info(
        { prId: pullRequestId, closedCount: closedIssueIds.length },
        "Issues closed on PR merge",
      );
    }
  } catch (error) {

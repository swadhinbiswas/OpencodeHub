...
export const workflows = pgTable(
  ...
  (t) => ({
    repoIdx: index("workflows_repo_idx").on(t.repositoryId),
  }),
);
...
export const workflowRuns = pgTable(
  ...
  (t) => ({
    ...
    repoIdx: index("workflow_runs_repo_idx").on(t.repositoryId),
...
export const workflowJobs = pgTable(
  ...
  (t) => ({
    runIdx: index("workflow_jobs_run_idx").on(t.runId),
  }),
);
...
export const workflowSecrets = pgTable("workflow_secrets", {
  ...
  repositoryId: text("repository_id")
    ...
    .references(() => repositories.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  encryptedValue: text("encrypted_value").notNull(), // AES-GCM encrypted at rest

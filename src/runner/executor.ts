...
const DEFAULT_LIMITS = {
  ...
  maxLogBytes: parseInt(
    process.env.EXECUTOR_MAX_LOG_BYTES || String(10 * 1024 * 1024),
    10,
  ),
};

// ── Image Allowlist ───────────────────────────────────────────────────────────
// Only these image prefixes are permitted for job containers.
// Set EXECUTOR_IMAGE_ALLOWLIST env var to override (comma-separated).
...
const IMAGE_ALLOWLIST: string[] = process.env.EXECUTOR_IMAGE_ALLOWLIST
  ? process.env.EXECUTOR_IMAGE_ALLOWLIST.split(",").map((s) => s.trim())
...
export class Executor {
  ...
  async verifyDocker() {
    try {
      ...
      logger.error(
        "Docker is NOT available. Please install Docker or ensure the runner has permissions.",
      );
      return false;
    }
  }
  ...
  /**
   * Validate that the requested image is in the allowlist
   */
  private validateImage(image: string): boolean {
    return IMAGE_ALLOWLIST.some((prefix) => image.startsWith(prefix));
  }

  /**
   * Encrypt sensitive environment variables for at-rest storage
   */
  ...
  /**
   * Decrypt credentials that were encrypted with encryptCredential
   */
  static decryptCredential(ciphertext: string): string {
    if (!ciphertext.startsWith("enc:")) return ciphertext;
    if (!CREDENTIAL_ENCRYPTION_KEY || CREDENTIAL_ENCRYPTION_KEY.length !== 64) {
      logger.warn(
        "Encrypted credential found but EXECUTOR_CREDENTIAL_KEY not set",
      );
      return ciphertext;
    }
  ...
  async runJob(
    ...
    if (!this.validateImage(image)) {
      logger.error(
        { image, allowlist: IMAGE_ALLOWLIST },
      ...
      return {
        ...
        exitCode: 1,
        logs:
          `Security Error: Image "${image}" is not in the allowed images list. ` +
          `Contact your administrator to add it to EXECUTOR_IMAGE_ALLOWLIST.`,
      };
    }

    // Decrypt env vars that may be encrypted
    const decryptedEnv: Record<string, string> = {};
    if (options.env) {
      for (const [k, v] of Object.entries(options.env)) {
        decryptedEnv[k] = Executor.decryptCredential(v);
      }
    }
    ...
    try {
      ...
      container = await this.docker.createContainer({
        ...
        name: containerName,
        Env:
          Object.keys(decryptedEnv).length > 0
            ? Object.entries(decryptedEnv).map(([k, v]) => `${k}=${v}`)
            : options.env
        ...
        HostConfig: {
          ...
          // Network isolation: "none" blocks all network access
      ...
      if ("timedOut" in outcome && outcome.timedOut) {
        logger.warn(
          { jobId, containerName, timeoutSeconds: limits.timeoutSeconds },
